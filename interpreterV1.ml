  ...
  |
    ...
    if n = 0 then
      ...
      match s with 
        ...
        let new_stack_tail = (drop n s) in 
        begin
          match h, ints_of_Ints rest with
            Int i, Some ints ->  
            let new_stack_head = 
              Int (i - (List.fold_left (+) 0 ints)) in 
            let new_stack = new_stack_head::new_stack_tail in 
            run t new_stack output
  ...
  |
    ...
    if n = 0 then
      ...
      match ints_of_Ints (take n s) with 
        ...
        Some ints ->  
        let new_stack_tail = (drop n s) in 
        let new_stack_head = Int (List.fold_right ( * ) ints 1) in
        let new_stack = new_stack_head::new_stack_tail in 
        run t new_stack output  
      |
        None -> ["Error"]
    ...
    if n = 0 then
      ...
      match s with 
        ...
        begin
          match h, ints_of_Ints rest with
            Int i, Some ints ->  
            let p = (List.fold_left ( * ) 1 ints) in
            if p = 0 then ["Error"]
            else let new_stack = Int (i/p)::new_stack_tail in 
              run t new_stack output
